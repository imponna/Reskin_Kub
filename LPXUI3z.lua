if game.PlaceId == 2753915549 or game.PlaceId == 4442272183 or game.PlaceId == 7449423635 then
    ...
    local LPXHUB = Instance.new("ScreenGui")
    local LPXTOGGLE = Instance.new("ImageButton")
    local UICorner = Instance.new("UICorner")
    local TweenService = game:GetService("TweenService")
    ...
    local rotateGoal = { Rotation = 360 } -- หมุน 360 องศา
    local rotateTween = TweenService:Create(LPXTOGGLE, tweenInfo, rotateGoal)
    LPXHUB.Parent = game:GetService("CoreGui")
    LPXHUB.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    LPXTOGGLE.Parent = LPXHUB
    LPXTOGGLE.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    LPXTOGGLE.BorderSizePixel = 0
    LPXTOGGLE.Position = UDim2.new(0.01, 0, 0.13, 0)
    LPXTOGGLE.Size = UDim2.new(0, 40, 0, 40)
    LPXTOGGLE.Image = "rbxassetid://77755869678226"
    UICorner.Parent = LPXTOGGLE
    UICorner.CornerRadius = UDim.new(0, 10)
    local ClickSound = Instance.new("Sound")
    ClickSound.Parent = LPXTOGGLE
    ClickSound.SoundId = "rbxassetid://3428115723"
    LPXTOGGLE.MouseButton1Down:Connect(function()
    ClickSound:Play()
    game:GetService("VirtualInputManager"):SendKeyEvent(true,305,false,game)
    game:GetService("VirtualInputManager"):SendKeyEvent(false,305,false,game)
    rotateTween:Play()
    rotateTween.Completed:Wait()
    local resetGoal = { Rotation = 0 }
    local resetTween = TweenService:Create(LPXTOGGLE, tweenInfo, resetGoal)
    resetTween:Play()
    end)
    --------------------------------------------------------------------
    local LPXUi = library:AddWindow("LPX Hub | BF",Enum.KeyCode.RightControl)
    --------------------------------------------------------------------
    local Main = LPXUi:AddTab("Auto Farm","")
    local Combat = LPXUi:AddTab("Combat","")
    local Stats = LPXUi:AddTab("Stats","")
    local Teleport = LPXUi:AddTab("Teleport","")
    local Dungeon = LPXUi:AddTab("Dungeon","")
    local DevilFruit = LPXUi:AddTab("Devil Fruit","")
    local Shop = LPXUi:AddTab("Shop","")
    local Misc = LPXUi:AddTab("Misc","")
    --------------------------------------------------------------------
